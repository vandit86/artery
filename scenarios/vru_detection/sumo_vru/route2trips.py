...
class RouteReader(handler.ContentHandler):
    ...
    def __init__(self, attrList, outfile, vias, calledBy=""):
        ...
        self.personType = 'type="DEFAULT_PEDTYPE"'
    ...
    def endElement(self, name):
        if name == 'route' or name == 'walk':
            if not self._vID:
                self._routes[self._routeID] = self._routeString
                self._routeString = ''
            self._routeID = ''
        elif name == 'vehicle' or name == 'person':
            ...
            if self._attrList:
                ...
                if name== 'vehicle': 
                    print('    <trip id="%s" %s%s/>' % (self._vID, ' '.join(items), via),file=self.outfile)
                elif name == 'person': 
                    print('    <trip id="%s" %s%s %s/>' % (self._vID, ' '.join(items), via, self.personType),file=self.outfile)
            self._vID = ''
            self._routeString = ''
        elif name == 'routes':
